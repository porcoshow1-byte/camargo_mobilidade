...
// Inline Logo Component — totalmente transparente, sem dependência de arquivo de imagem horizontal
const BrandLogo = ({ size = 'md' }: { size?: 'sm' | 'md' | 'lg' | 'xl' }) => {
  const iconSize = size === 'sm' ? 'h-8 w-8' : size === 'lg' ? 'h-12 w-12 md:h-14 md:w-14' : size === 'xl' ? 'h-16 w-16' : 'h-10 w-10';
  const textSize = size === 'sm' ? 'text-sm' : size === 'lg' ? 'text-xl md:text-2xl' : size === 'xl' ? 'text-3xl' : 'text-base';
  const subSize = size === 'sm' ? 'text-[9px]' : size === 'lg' ? 'text-xs md:text-sm' : size === 'xl' ? 'text-base' : 'text-[11px]';
  return (
    <div className="flex items-center gap-2.5 select-none">
      <img src="/icon.png" alt="" aria-hidden="true" className={`${iconSize} object-contain flex-shrink-0 drop-shadow-[0_2px_8px_rgba(245,183,49,0.4)]`} />
      <div className="flex flex-col leading-none">
        <span className={`font-black tracking-wide text-[#F5B731] ${textSize} uppercase`}>Camargo</span>
        <span className={`font-bold tracking-widest text-white ${subSize} uppercase`}>Mobilidade</span>
      </div>
    </div>
  );
};
...
export const LandingPage = ({ onStartDemo }: { onStartDemo: () => void }) => {
  const phoneRaw = "14997557224";
  const phoneDisplay = "(14) 99755-7224";
  const whatsappLink = `https://wa.me/55${phoneRaw}`;
  const callLink = `tel:${phoneRaw}`;
  const mapLink = "https://www.google.com/maps/search/?api=1&query=R.+dos+Expedicion�rios,+255+-+Centro,+Ourinhos+-+SP,+19900-041";
  ...
  const benefits = [
    {
      ...
      text: "Chegue mais rápido aos seus compromissos com um atendimento ágil e prático em Ourinhos.",
    },
    {
      icon: ShieldCheck,
  ...
  return (
    <div className="min-h-screen bg-[#0A1628] text-white font-sans selection:bg-[#F5B731] selection:text-black">
      <section className="relative overflow-hidden">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(245,183,49,0.22),transparent_35%),radial-gradient(circle_at_bottom_right,rgba(255,255,255,0.06),transparent_30%)] pointer-events-none" />
        <div className="mx-auto max-w-7xl px-6 py-6 sm:px-8 lg:px-10">
          <div className="flex items-center justify-between rounded-full border border-white/10 bg-white/5 px-4 py-3 backdrop-blur z-20 relative">
            ...
            <div className="hidden md:flex items-center gap-3">
              ...
              <a href={callLink}>
                <button className="rounded-full bg-white text-[#0A1628] hover:bg-gray-100 font-semibold px-4 py-2 transition-all">Ligar agora</button>
              </a>
            </div>
          </div>
        </div>
        ...
        <div className="relative mx-auto grid max-w-7xl gap-12 px-6 pb-20 pt-8 sm:px-8 lg:grid-cols-[1.1fr_0.9fr] lg:px-10 lg:pb-28 lg:pt-10 z-10">
          <div className="max-w-2xl animate-fade-in">
            <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-[#F5B731]/30 bg-[#F5B731]/10 px-4 py-2 text-sm text-[#FEF3D0]">
              <Star className="h-4 w-4 text-[#F5B731]" />
              Mobilidade ágil e atendimento confiável em Ourinhos - SP
            </div>

            <h1 className="text-4xl font-black leading-tight tracking-tight text-white sm:text-5xl lg:text-6xl animate-slide-up">
              Mototáxi premium para quem precisa de <span className="text-[#F5B731]">rapidez</span>,
              <span className="text-[#F5D06B]"> praticidade</span> e confiança.
            </h1>

            <p className="mt-6 max-w-xl text-lg leading-8 text-white/75 sm:text-xl animate-slide-up" style={{ animationDelay: "100ms" }}>
              O <strong className="text-white">Camargo Mobilidade</strong> leva você com mais agilidade pela cidade e também realiza
              <strong className="text-[#FEF3D0]"> pequenas entregas</strong> com eficiência. Peça sua corrida pelo telefone ou baixe o app.
            </p>

            <div className="mt-8 flex flex-col gap-4 sm:flex-row animate-slide-up" style={{ animationDelay: "200ms" }}>
              <a href={appLink}>
                <button className="flex items-center justify-center h-12 w-full sm:w-auto rounded-full bg-[#F5B731] px-7 text-base font-semibold text-[#0A1628] hover:bg-[#E5A420] transition-all">
                  Baixar o app
            ...
            <div className="mt-10 grid grid-cols-2 gap-4 sm:grid-cols-4 animate-slide-up" style={{ animationDelay: "300ms" }}>
              {[
                ...
                <div key={item} className="rounded-2xl border border-white/10 bg-[#0F1D32]/50 p-4 text-sm font-medium text-white/80 backdrop-blur text-center flex items-center justify-center">
                  {item}
                </div>
              ))}
            </div>
          ...
          <div className="lg:pl-4 animate-slide-up" style={{ animationDelay: "300ms" }}>
            <div className="rounded-[32px] border border-white/10 bg-gradient-to-b from-white/10 to-white/5 p-5 shadow-2xl shadow-black/30 backdrop-blur">
              <div className="rounded-[28px] border border-white/10 bg-[#111F38] p-6">
                <div className="mb-5 flex items-center justify-between">
                  <div>
                    <p className="text-sm uppercase tracking-[0.25em] text-[#D4A53A] font-semibold">Camargo Mobilidade</p>
                    <p className="mt-2 text-2xl font-bold">Rápido. Seguro. Profissional.</p>
                  </div>
                  <img src="/icon.png" alt="Logo" className="h-14 w-14 drop-shadow-lg" />
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="rounded-2xl bg-[#F5B731]/12 p-4">
                      <p className="text-sm font-medium text-[#FDE68A]">Contato</p>
                      <p className="mt-1 font-semibold lg:text-lg">{phoneDisplay}</p>
                    </div>
                    <div className="rounded-2xl bg-[#F5B731]/12 p-4">
                      <p className="text-sm font-medium text-[#FDE68A]">Local</p>
                      <p className="mt-1 font-semibold lg:text-lg">Centro de Ourinhos</p>
                    </div>
                  </div>
                  <div className="rounded-2xl border border-[#F5B731]/20 bg-[#F5B731]/10 p-4 text-sm leading-7 text-[#FEF3D0]">
      ...
      <section className="mx-auto max-w-7xl px-6 py-6 sm:px-8 lg:px-10">
        <div className="grid gap-4 rounded-[28px] border border-white/10 bg-white/[0.04] p-5 md:grid-cols-4">
          {trafficHighlights.map((item) => (
            <div key={item} className="rounded-2xl bg-[#0F1D32] border border-white/5 p-4 text-sm font-medium text-white/75 text-center">
              {item}
            </div>
          ))}
        </div>
      ...
      <section className="mx-auto max-w-7xl px-6 py-20 sm:px-8 lg:px-10">
        <div className="max-w-3xl">
          <p className="text-sm font-semibold uppercase tracking-[0.3em] text-[#C99A2E]">Por que escolher</p>
          ...
          <p className="mt-5 text-lg leading-8 text-white/70">
            Quando o tempo é curto, você precisa de um atendimento ágil, prático e confiável. O Camargo Mobilidade ajuda você a se deslocar com rapidez e também resolve pequenas entregas sem complicação.
          </p>
        </div>
          ...
          {benefits.map(({ icon: Icon, title, text }) => (
            <Card key={title} className="rounded-[28px] border border-white/10 bg-white/[0.04] text-white shadow-xl shadow-black/20 hover:bg-white/[0.06] transition-colors">
              <CardContent className="p-8">
                <div className="mb-6 flex h-14 w-14 items-center justify-center rounded-2xl bg-[#F5B731] text-[#0A1628]">
      ...
      <section className="border-y border-white/10 bg-[#0F1D32] overflow-hidden">
        <div className="mx-auto grid max-w-7xl gap-12 px-6 py-20 sm:px-8 lg:grid-cols-2 lg:px-10">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.3em] text-[#C99A2E]">Serviços</p>
            <h2 className="mt-4 text-3xl font-black tracking-tight sm:text-4xl leading-tight">
              Corridas e pequenas entregas com atendimento rápido em Ourinhos.
            </h2>
            <p className="mt-5 text-lg leading-8 text-white/70">
              Seja para chegar logo ao seu destino ou enviar uma pequena encomenda com praticidade, o Camargo Mobilidade oferece uma solução moderna para o dia a dia.
            </p>
            <div className="mt-8 grid gap-4">
              {services.map((service) => (
                <div key={service} className="flex items-center gap-4 rounded-2xl border border-white/10 bg-white/5 p-4 transition hover:bg-white/10">
                  <div className="flex h-8 w-8 items-center justify-center rounded-full bg-[#F5B731] text-[#0A1628] shrink-0">
          ...
          <div className="rounded-[32px] border border-white/10 bg-gradient-to-br from-[#F5B731]/18 to-white/5 p-6 shadow-2xl shadow-black/20 h-auto self-center">
            <div className="h-full rounded-[28px] border border-white/10 bg-[#0A1628] p-8 lg:p-10">
              <p className="text-sm font-semibold uppercase tracking-[0.3em] text-[#D4A53A]">Contato Direto</p>
              <h3 className="mt-4 text-3xl font-black leading-tight">Precisa chegar rápido ou fazer uma entrega?</h3>
              <p className="mt-4 text-lg leading-8 text-white/75">
                Chame o <strong className="text-white">Camargo Mobilidade</strong>. Atendimento ágil em Ourinhos para corridas e pequenas entregas com praticidade no seu dia.
              </p>
              ...
              <div className="mt-8 space-y-3">
                {[
                  ...
                  <div key={bullet} className="rounded-2xl bg-white/5 border border-white/5 px-4 py-3 text-white/80 font-medium flex gap-3 items-center">
                    <CheckCircle className="h-5 w-5 text-[#F5B731]" /> {bullet}
                  </div>
                ))}
              </div>
              ...
              <div className="mt-8 flex flex-col gap-4 sm:flex-row">
                <a href={callLink} className="w-full">
                  <button className="h-14 w-full rounded-full bg-[#F5B731] px-6 text-base font-bold text-[#0A1628] hover:bg-[#E5A420] transition-all relative overflow-hidden group">
      ...
      <section id="baixar-app" className="mx-auto max-w-7xl px-6 py-20 sm:px-8 lg:px-10">
        <div className="grid gap-8 rounded-[32px] border border-white/10 bg-gradient-to-r from-[#F5B731]/16 to-white/5 p-8 lg:grid-cols-[1fr_0.8fr] lg:p-12 shadow-2xl shadow-[#F5B731]/5">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.3em] text-[#D4A53A]">App Camargo Mobilidade</p>
            <h2 className="mt-4 text-4xl font-black tracking-tight leading-tight">
              Baixe o app e tenha mais praticidade.
            </h2>
            <p className="mt-6 max-w-2xl text-lg leading-8 text-white/75">
              Facilite sua rotina com uma experiência moderna. Solicite corridas com mais comodidade e tenha o Camargo Mobilidade sempre à mão, direto no seu smartphone.
            </p>
            <div className="mt-10 flex flex-col gap-4 sm:flex-row">
              <button className="h-14 w-full sm:w-auto rounded-full bg-[#F5B731] px-8 text-base font-bold text-[#0A1628] hover:bg-[#E5A420] transition-all shadow-lg shadow-[#F5B731]/20">
                Baixar app Android
              </button>
          ...
          <div className="rounded-[28px] border border-white/10 bg-[#0F1D32] p-6 lg:ml-auto w-full md:w-[360px]">
            <div className="rounded-[24px] bg-white/5 p-8 h-full flex flex-col justify-center border border-white/5 shadow-inner">
               <img src="/icon.png" alt="Logo Camargo Mobilidade" className="w-44 h-44 mx-auto drop-shadow-[0_20px_30px_rgba(245,183,49,0.3)] mb-6" />
               <p className="text-center font-bold text-2xl text-white">Camargo Mobilidade</p>
      ...
      <section className="mx-auto max-w-7xl px-6 pb-24 sm:px-8 lg:px-10">
        <div className="grid gap-8 lg:grid-cols-[0.95fr_1.05fr]">
          <div className="rounded-[30px] border border-white/10 bg-white/[0.04] p-8 sm:p-10 shadow-lg">
            <p className="text-sm font-semibold uppercase tracking-[0.3em] text-[#C99A2E]">Contato</p>
            <h2 className="mt-4 text-3xl font-black tracking-tight">Fale com o Camargo Mobilidade</h2>
            <div className="mt-8 space-y-6 text-white/75">
              <div className="flex items-start gap-4">
                <div className="rounded-2xl bg-[#162748] border border-[#F5B731]/30 p-4 text-[#F5B731] shadow-lg"><Phone className="h-6 w-6" /></div>
                <div className="pt-1">
              ...
              <div className="flex items-start gap-4">
                <div className="rounded-2xl bg-[#162748] border border-[#F5B731]/30 p-4 text-[#F5B731] shadow-lg"><MapPin className="h-6 w-6" /></div>
                <div className="pt-1">
                  <p className="text-sm font-medium text-white/50 text-transform uppercase tracking-wider">Endereço</p>
                  <p className="text-xl font-bold text-white mt-1">R. dos Expedicion�rios, 255</p>
                  <p className="text-lg">Centro, Ourinhos - SP, 19900-041</p>
                </div>
              </div>
            </div>
            ...
            <div className="mt-10 flex flex-col gap-4 sm:flex-row">
              <a href={callLink} className="w-full sm:w-auto">
                <button className="h-14 w-full rounded-full bg-[#F5B731] px-8 text-base font-bold text-[#0A1628] hover:bg-[#E5A420] transition-all shadow-lg shadow-[#F5B731]/20 hover:shadow-none translate-y-0 hover:translate-y-1">Ligar agora</button>
              </a>
          ...
          <div className="rounded-[30px] border border-white/10 bg-gradient-to-br from-white/5 to-[#F5B731]/10 p-4">
            <div className="flex h-full min-h-[320px] items-center justify-center rounded-[26px] overflow-hidden border border-white/10 bg-[#0b140f] relative group shadow-inner">
              ...
              <div className="relative z-20 text-center flex flex-col items-center">
                 <div className="bg-[#F5B731] p-5 rounded-full mb-5 shadow-2xl text-[#0A1628] ring-8 ring-[#F5B731]/20">
                   <MapPin size={32} />
                 </div>
                 <p className="font-extrabold text-3xl text-white drop-shadow-md">Ourinhos - SP</p>
      ...
      <footer className="border-t border-white/10 bg-[#050d09] py-12 md:py-16">
        <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-10 text-center text-white/50 text-sm flex flex-col items-center gap-6">
           <div className="flex justify-center mb-2 p-3 bg-white/5 rounded-2xl border border-white/5">
               <BrandLogo size="sm" />
           </div>
           <p className="text-base">&copy; {new Date().getFullYear()} Camargo Mobilidade. Ourinhos - SP. <br className="md:hidden"/> Todos os direitos reservados.</p>
      ...
      <a
        ...
        target="_blank"
        rel="noreferrer"
        className="fixed bottom-6 right-6 z-50 rounded-full bg-[#F5B731] px-6 py-4 text-base font-extrabold text-[#0A1628] shadow-2xl shadow-[#F5B731]/40 transition-all hover:scale-105 hover:bg-[#E5A420] flex items-center gap-2"
      >
