...
const getMockData = (): Company[] => {
    try {
        const stored = JSON.parse(localStorage.getItem('camargomobilidade_mock_companies') || '[]');
        return stored.length > 0 ? stored : INITIAL_COMPANIES;
    } catch { return INITIAL_COMPANIES; }
};

const saveMockData = (companies: Company[]) => {
    localStorage.setItem('camargomobilidade_mock_companies', JSON.stringify(companies));
};
