...
const createRealPixPayment = async (rideId: string, price: number, email: string, accessToken?: string): Promise<PixPaymentResponse> => {
  ...
  const response = await fetch('https://api.mercadopago.com/v1/payments', {
    ...
    body: JSON.stringify({
      ...
      payer: {
        email: email || 'passenger@camargomobilidade.com'
      },
      description: `Corrida Camargo Mobilidade #${rideId.slice(0, 6)}`,
...
export const createPixPayment = async (rideId: string, price: number, email: string, options?: { accessToken?: string }) => {
  ...
  if (isReal) {
    ...
    return {
      id: "sim_" + rideId,
      status: "pending",
      qr_code: "00020126580014BR.GOV.BCB.PIX0136123e4567-e89b-12d3-a456-426614174000520400005303986540510.005802BR5917Camargo Mobilidade6008Ourinhos62070503***6304ABCD",
